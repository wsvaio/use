...
export function useCalendar() {
  ...
  // 前一月结束日期
  const beforeMonthEndDay = ref(0);
  // 当前月结束日期
  const currMonthEndDay = ref(0);
  // 后一月结束日期
  const afterMonthEndDay = ref(0);
  // 当前月开始星期数
  const currMonthStartWeek = ref(0);
  ...
  const dayNumFormat = (num: number) => {
    ...
    if (num <= 0)
      num = beforeMonthEndDay.value + num;
    else if (num > currMonthEndDay.value)
      num = num - currMonthEndDay.value;
    return num;
  };
