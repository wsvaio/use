...
export const usePayloadKey = Symbol("usePayloadKey");
...
/**
 * @deprecated payload 和 action 的组合过于抽象，将其分离是更好的选择（已拆分为useAction，usePload），usePayload之后不再更新，仅作bug修复
 * Payload Hook
 * 用于在 Vue 组件之间共享状态和实现组件通信
 */
export function usePayload<Initial extends object, Actions extends Record<string, object> = Record<string, object>>(initial = {} as (
  | (Initial & { $mode?: "provide"; $key?: string | symbol })
  | (Partial<Initial> & { $mode?: "" | "inject" | "auto"; $key?: string | symbol })
) &
Record<any, any>): Payload<{ Initial: Omit<Initial, "$mode" | "$key">; Actions: Actions }> {
  /**
   * 模式和键名
   */
  let { $mode, $key = usePayloadKey } = pick(initial, ["$mode", "$key"], true) as {
        ...
        if (names.length) {
          ...
          for (const name of names) {
            ...
            if (!vs.some(v => v.has(name))) {
              ...
              if (!options)
                payload.$options.set(name, (options = []));
              options.push(option);
            }
          }
          ...
          [actions, payload.$actions].forEach(map => {
            ...
            names.forEach((name: string) => {
              ...
              if (_options) {
                ...
                if (!_options.length)
                  payload.$options.delete(name);
              }

              middlewares.forEach(middleware => {
                let set = map.get(middleware);
                if (!set)
                  map.set(middleware, (set = new Set()));
                set.add(name);
              });
            });
            // @ts-expect-error 这里并不需要类型检测
      ...
      (...names) =>
        (...middlewares) => {
          [actions, payload.$actions].forEach(map =>
            names.forEach((name: string) => {
              middlewares.forEach(middleware => {
                const set = map.get(middleware);
                if (!set)
                  return;
                set.delete(name);
                if (set.size <= 0)
                  map.delete(middleware);
              });
            })
          );
        },
